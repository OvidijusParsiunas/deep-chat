...
export class UploadFileButton extends ButtonStyleEvents<Styles> {
  ...
  constructor(containerElement: HTMLElement, fileAttachments: FileAttachments, images: FileServiceIO, id: string) {
    super(UploadFileButton.createImageElement(), typeof images.button === 'object' ? images.button : {});
    this._innerElements = UploadFileButton.createInnerElements(id, this._customStyles);
  ...
  private static createInnerElements(id: string, customStyles?: Styles) {
    const baseButton = UploadFileButton.createSVGIconElement(id);
    return {
