  ...
  public static create(deepChat: DeepChat): ServiceIO {
    ...
    if (connect && Object.keys(connect).length > 0 && !demo) {
      ...
      if (connect.url) {
        ...
        // Endpoint routing based on URL pattern: http://host:port/service-model-model2
        // Example URLs:
        // - http://host:port/openai-chat => OpenAI chat service
        // - http://host:port/openai-tts => OpenAI text-to-speech service
        // - http://host:port/huggingface-audio-speech => HuggingFace audio speech recognition
        switch (name) {
          case 'openai':
            switch (model) {
              case 'image':
