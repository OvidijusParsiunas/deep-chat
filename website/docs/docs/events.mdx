...
Triggered when the [`clearMessages`](./methods#clearMessages) method has been executed.

#### Example

<ComponentContainerEvents propertyName={'onMessagesCleared'} withMethod={true}>
...
```html
chatElementRef.onMessagesCleared = () => { console.log("Messages cleared"); };
```
