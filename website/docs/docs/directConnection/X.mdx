...
- Type: `true` | \{<br />
  ...
  \}
- Default: _\{model: "grok-4.3"\}_

Connect to X's [Responses API](https://docs.x.ai/developers/model-capabilities/text/generate-text). You can set this property to _true_ or configure it using an object: <br />
`model` is the name of the X model to be used by the API. <br />
`max_output_tokens` limits the maximum number of tokens in the generated response. <br />
...
- Type: `true` | \{`model?: string`,`n?: number`,`size?: string`}
- Default: _\{model: "grok-imagine-image-quality"\}_

Connect to X's [Images Generation API](https://docs.x.ai/developers/model-capabilities/imagine). You can set this property to _true_ or configure it using an object: <br />
`model` is the name of the image generation model. <br />
`n` is the number of images to generate. <br />
`size` is the image dimensions or aspect ratio. <br />
