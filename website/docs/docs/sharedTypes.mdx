...
### `ButtonElementStyles` {#ButtonElementStyles}

- Type: {<br />
  &nbsp;&nbsp;&nbsp;&nbsp; [`container?: StatefulStyles`](#StatefulStyles), <br />
  &nbsp;&nbsp;&nbsp;&nbsp; [`svg?: ButtonInnerElementStyles`](#ButtonInnerElementStyles), <br />
  &nbsp;&nbsp;&nbsp;&nbsp; [`text?: ButtonInnerElementStyles`](#ButtonInnerElementStyles) <br />
  }
...
### `ButtonInnerElementStyles` {#ButtonInnerElementStyles}
...
`content` is a string that is used to generate the inner element. When it is inside a [`text`](#ButtonElementStyles) property - the string represents the element text.
When inside an [`svg`](#ButtonElementStyles) property - the string is the full inlined element HTML.

[Fully editable example for SVGs](HERE)

:::info
If the `content` string is defined for an [`svg`](#ButtonElementStyles):

- It needs to contain a _xmlns="http://www.w3.org/2000/svg"_ attribute.
- If it is inside a HTML attribute (full object is inside quotes `"` - see the editable example) or when not using backticks (`` ` ``) - make sure to escape the quotes by changing
  ...
  <AiAssistantBrowser
    service={{
      custom: {
        ...
        microphoneAudio: {button: {position: 'outside-right'}},
      },
    }}
  ></AiAssistantBrowser>
</ComponentContainer>
