...
<h2>How to connect to a Custom API</h2>

Begin by defining the target api endpoint and other configuration inside the [`request`](#request) property.
...
can stream the response messages into the chat. This is only supported for text messages. It is IMPORTANT for the final event message to contain the following string: _"[DONE]"_.
