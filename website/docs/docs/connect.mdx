...
- Type: {`text?: string`, [`files?: MessageFile[]`](./messages#MessageFile), `html?: string`, `error?: string`}
...
`html` is a string that defines the markup for [message elements](./messages/HTML). <br />
...
This is achieved via the use of the `"deep-chat-update-message"` class. Checkout an [example](./messages/HTML#example-for-deep-chat-update-message) in [Deep Chat Classes](./messages/HTML#deepChatClasses).
:::
