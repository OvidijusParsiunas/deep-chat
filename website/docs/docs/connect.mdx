...
- Type: `boolean` | {`simulation?: boolean`}

Used to stream text responses. <br />
If the target service supports [`server-sent events`](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events) via the `text/event-stream` MIME type,
...
Use the `simulation` object property to facilitate a <b>stream-like</b> experience for regular HTTP responses.
...
```html
<deep-chat stream='{"simulation": true}'></deep-chat>
```
