...
Styling for the chat's container element. It can either be defined as the usual [style attribute](https://www.w3schools.com/tags/att_style.asp)
or as an object using your framework's syntax.<br />
...
`stop` is used when a message response is being [`streamed`](./connect#stream). <br />
`disabled` is used when the user's input cannot be sent (see [`validateInput`](./interceptors#validateInput)) or the [websocket](./connect#Websocket) connection is not live.
You can also set this state manually by calling the [`disableSubmitButton`](./methods#disableSubmitButton) method. <br />
...
:::info
When [displayLoadingBubble](./messages/#displayLoadingBubble) is set to _false_ and the `loading` property's `content` string for `text` or `svg` is not set,
