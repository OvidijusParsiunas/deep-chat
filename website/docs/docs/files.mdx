...
// TO-DO add examples of actual files
...
<ComponentContainer>
  <DeepChatBrowser
    ...
    initialMessages={[{file: {src: '/text/text.txt', type: 'file'}, role: 'user'}]}
    existingService={{demo: true}}
    mixedFiles={true}
  ></DeepChatBrowser>
</ComponentContainer>
...
<deep-chat
  ...
  containerStyle='{"borderRadius": "8px"}'
  introMessage="Use the file button or drop a file to attach it to the next outgoing message."
  initialMessages='[{"file": {"src": "path-to-file.txt", "type": "file"}, "role": "user"}]'
></deep-chat>
```
