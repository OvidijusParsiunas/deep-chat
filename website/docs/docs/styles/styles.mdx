...
- Type: \{`text?: string`, `files?: File[] | FileList`\}

Input contents displayed when the chat renders. <br />
`text` is displayed in the text input. <br />
`files` are displayed in the file attachment container. This can either be an array
...
<ComponentContainer>
  <DeepChatBrowser style={{borderRadius: '8px'}} defaultInput={{text: 'Default text'}} demo={true}></DeepChatBrowser>
</ComponentContainer>
...
```html
<deep-chat defaultInput='{"text": "Default text"}'></deep-chat>
```
...
<deep-chat defaultInput='{"text": "Default text"}' style="border-radius: 8px" demo="true"></deep-chat>
```
