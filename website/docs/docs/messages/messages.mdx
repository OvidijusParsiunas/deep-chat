...
is not taken into consideration when calculating [`requestBodyLimits`](../connect#requestBodyLimits). <br />
`html` is used to render custom code - [more info](./html).
...
- Type: {[`role: MessageRole`](#MessageRole), `text?: string`, [`files?: MessageFile[]`](#MessageFile), [`html?: string`](./HTML)}

Object format used to encapsulate a message within the chat. The `role` property is required whilst other properties are defined/combined depending
on what is being transferred in the message. <br />
The `text` string uses [Markdown](https://www.markdownguide.org/basic-syntax/) syntax to render the message accordingly. This is useful for displaying code snippets, paragraphs etc. See this [playground](https://jonschlinkert.github.io/remarkable/demo/) for reference. <br />
...
Entity that the message was sent from.
