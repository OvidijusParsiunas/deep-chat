...
Here is an example for [`initialMessages`](/docs/messages#initialMessages):
...
`deep-chat-update-message` - used in [websockets](/docs/connect#Websocket) to allow the server to update the last AI `html` message.
This class needs to be applied to the parent-most element in the `html` string. When tracking state, we recommend calling
the [getMessages](/docs/methods#getMessages) method when the [onNewMessage](/docs/events#onNewMessage) event is triggered. <br />
...
This example uses a GIF as it needs a live [websocket](/docs/connect#Websocket) connection.
