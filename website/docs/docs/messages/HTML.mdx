...
If you want to use your code for an introduction panel - check out [`Intro Panel`](../introPanel) instead.

### Getting started {#Getting Started}

The `html` property can be used in server [Response](../connect#Response)s and is stored inside the chat using the [MessageContent](../messages#MessageContent) format.
Here is an example for [`initialMessages`](../messages#initialMessages):
...
- Type: {<br />
  ...
  &nbsp;&nbsp;&nbsp;&nbsp; [`styles?: StatefulStyles`](./styles/#StatefulStyles) <br />
  }}
...
<ComponentContainer>
  <DeepChatBrowser
    ...
    initialMessages={[
      {html: '<button class="custom-button">Hello</button>', role: 'user'},
      {html: '<button class="custom-button ai-button">Goodbye</button>', role: 'ai'},
    ]}
    style={{height: '370px', borderRadius: '8px'}}
  ></DeepChatBrowser>
</ComponentContainer>
...
chatElementRef.htmlClassUtilities = {
  ['custom-button']: {
    events: {
      mousedown: (event) => (event.target.innerText = 'clicked'),
      mouseenter: (event) => (event.target.innerText = 'hovered'),
    },
    styles: {
      default: {padding: '3px 8px', cursor: 'pointer'},
      hover: {backgroundColor: 'yellow'},
...
`deep-chat-update-message` - used in [websockets](../connect#Websocket) to allow the server to update the last AI `html` message.
This class needs to be applied to the parent-most element in the `html` string. When tracking state, we recommend calling
the [getMessages](../methods#getMessages) method when the [onNewMessage](../events#onNewMessage) event is triggered. <br />
...
#### Example for `deep-chat-update-message`

This example uses a GIF as it needs a live [websocket](../connect#Websocket) connection.

<img src={WebSocket} style={{marginLeft: 'auto', marginRight: 'auto', display: 'flex'}} />
...
1: {html: '<div class="deep-chat-update-message">Downloading...</div>'}
2: {html: '<div class="deep-chat-update-message">Loading...</div>'}
3: {html: '<div class="deep-chat-update-message">Processing...</div>'}
4: {html: '<div class="deep-chat-update-message">Ready...</div>'}
```
...
To unset/change the message bubble styling, use the `html` property in the [messageStyles](./styles#messageStyles) object.
...
<ComponentContainer>
  <DeepChatBrowser
    ...
    submitButtonStyles={{submit: {container: {default: {opacity: 0}}}}}
    demo={{
      response: {
        text: 'What about now?',
        html: `
...
chatElementRef.submitButtonStyles = {submit: {container: {default: {opacity: 0}}}};
