...
however this method MUST return a [`Result`](./service#Result) type object (or an object structure returned by a selected service in the [`directConnection`](./directConnection) property).
