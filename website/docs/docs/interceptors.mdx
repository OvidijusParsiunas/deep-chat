...
- Type: ([`RequestDetails`](HERE)) => [`RequestDetails`](HERE)

Triggered before a message is sent and can be used to observe/alter the outgoing details. The method MUST return a [`RequestDetails`](HERE) object.
...
however this method MUST return a [`Result`](HERE) type object (or an object structure returned by a selected [`existingService`](HERE)).
...
<ComponentContainerInterceptors propertyName={'validateMessageBeforeSending'}>
  <DeepChatBrowser existingService={{demo: true}} mixedFiles={true}></DeepChatBrowser>
</ComponentContainerInterceptors>
