...
```html
chatElementRef.requestInterceptor = (request) => { console.log(request); return request; };
```
...
```html
chatElementRef.responseInterceptor = (response) => { console.log(response); return response; };
```
...
:::info
if the _response_ needs asynchronous processing, you can instead return a [`Promise`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise) object
and resolve it as: `resolve(response)`. The _response_ variable should use the [`Result`](./connect/#Result) format. Do not use the `reject` function as errors should be
referenced inside the _response_ variable e.g. `resolve({error: 'error'})`.
:::
...
```html
chatElementRef.validateMessageBeforeSending = (text, files) => { return text ? true : false };
```
