...
- If you are connecting to a server via the [`request`](HERE) property - the type will be [`Result`](./connect/#Result).
- If you are connecting via the [`directConnection`](HERE) property - the type will be defined by the chosen service API.
