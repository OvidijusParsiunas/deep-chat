...
- Type: (`response: any`) => [`Result`](./connect#Result)
...
however this method MUST return a [`Result`](./connect#Result) type object (or an object structure returned by a selected service in the [`directConnection`](./directConnection) property).
