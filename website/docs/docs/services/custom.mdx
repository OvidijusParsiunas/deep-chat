...
Properties used to configure AiAssistant to connect to a custom API.
...
Configuration requires either `request` or `demo` to be set. Below is an example of demo.
...
Property types for a custom service:
...
It MUST return a [`Result`](#Result) type object. This cannot be used for [`stream`](#stream) messages.
