...
- Type: {[`completions?: Completions`](HERE), [`images?: Images`](HERE), [`audio?: Audio`](HERE)}
...
- Type: `true` | {[`OpenAIConverse`](HERE), `systemPrompt?: string`}
...
`language` is the language used the input audio. Supplying the input language in [_ISO-639-1_](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes) format will improve accuracy and latency. (Only used for [_transcription_](HERE) based API). <br />
