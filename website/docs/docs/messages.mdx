...
<deep-chat introMessage="Hi I am your assistant, ask me anything!" existingService='{"demo": true}'></deep-chat>
```
...
or by defining an object with [`Avatar`](HERE) properties. The `default` property is applied to both roles.
...
or by defining an object with [`Name`](HERE) properties. The `default` property is applied to both roles. <br />
...
<ComponentContainer>
  <DeepChatBrowser
    existingService={{demo: {displayErrors: {default: true, service: true, speechToTextInput: true}}}}
    errorMessages={{
      overrides: {
