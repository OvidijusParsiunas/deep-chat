...
is for communication with the target service and `speechToText` is when there is a problem using the `speechToText`.
functionality. <br />
...
<ComponentContainer>
  <DeepChatBrowser
    containerStyle={{borderRadius: '8px'}}
    directConnection={{demo: {displayErrors: {default: true, service: true, speechToText: true}}}}
    errorMessages={{
      overrides: {
