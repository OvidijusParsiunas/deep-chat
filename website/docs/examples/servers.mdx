...
The following are links to template backend projects that are preconfigured to work with Deep Chat. Each one includes a set of endpoints for
handling text messages, file attachments and streaming responses - incl. variations for a `basic` server that you can
customize to fit your use-cases and an `OpenAI` server that can act as a proxy.
